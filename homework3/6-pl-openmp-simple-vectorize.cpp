...
#else
#endif
int64_t solve(int64_t N, const int64_t* __restrict__ A, const int64_t* __restrict__ B) {
    int64_t sum = B[N]+A[0]+B[0];
    sum+=B[0] + 3*A[0] + 2*B[N];
    sum/=2;

    int i=1, iPlusN=N+1, NMinusi=N-1, N2Minusi=2*N-1;
    ...
    for (int64_t i = 1; i < (N+1)/2; i++) {
        ...
        __builtin_prefetch(&B[i + N + prefetch_distance], 0, 3);
        __builtin_prefetch(&A[N - i - prefetch_distance], 0, 3);
        __builtin_prefetch(&B[N - i - prefetch_distance], 0, 3);
        __builtin_prefetch(&B[2*N - i - prefetch_distance], 0, 3);
#pragma openmp simd
    ...
    // #pragma omp parallel for schedule(static) reduction(+:sum)
    // for (int64_t i = 1; i < (N+1)/2; i++) {
    //     sum += ((B[i] + A[i] + B[iPlusN]) + B(NMinusi) + A(NMinusi) + B(N2Minusi)) & ~1;
    //     i++; iPlusN++; NMinusi--; N2Minusi--;
    // }
...
int main() {
    ...
    int64_t sum = solve(N, A.data(), B.data());
